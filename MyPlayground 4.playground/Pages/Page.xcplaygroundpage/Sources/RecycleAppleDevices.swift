...
class RecycleAppleDevices: SKScene {
    ...
    private func showMessages() {
        if let firstMessage = childNode(withName: "firstMessage"),
            let secondMessage = childNode(withName: "secondMessage"),
            let nextButton = childNode(withName: "next"),
            let balloon = childNode(withName: "balloon") {
            let wait0 = SKAction.wait(forDuration: 0.5)
            let wait1 = SKAction.wait(forDuration: 4.4)
            let wait2 = SKAction.wait(forDuration: 5.4)
            let wait3 = SKAction.wait(forDuration: 9.2)
